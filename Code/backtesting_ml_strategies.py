...
def get_features_and_target(feature_mode='combined'):
    odds_features = ['HomeOdds', 'DrawOdds', 'AwayOdds']
    l5_features = [
        'Home_Avg_Points_L5', 'Home_Avg_GoalsConceded_L5', 'Home_Avg_Shots_L5',
        'Home_Avg_ShotsTarget_L5', 'Home_Avg_Corners_L5',
        'Away_Avg_Points_L5', 'Away_Avg_GoalsConceded_L5', 'Away_Avg_Shots_L5',
        'Away_Avg_ShotsTarget_L5', 'Away_Avg_Corners_L5'
    ]
    if feature_mode == 'odds_only':
        features = odds_features
    elif feature_mode == 'l5_only':
        features = l5_features
    else:
        features = odds_features + l5_features
    target = 'FTR'
...
def get_models():
    xgb_base = xgb.XGBClassifier(objective='multi:softprob', num_class=3, eval_metric='mlogloss', use_label_encoder=False, random_state=42, n_jobs=-1)
    models = {
        'LogisticRegression': LogisticRegression(max_iter=1000, random_state=42, n_jobs=-1),
        'XGBoost': xgb_base,
        'XGBoost_Calibrated': CalibratedClassifierCV(estimator=xgb_base, method='isotonic', cv=3),
...
def analyze_performance(results_dict, le):
    ...
    for name, data in results_dict.items():
        ...
        # New Metrics
        ll = log_loss(y_true_onehot, probs)
        
        market_implied = 1.0 / df_res[['HomeOdds', 'DrawOdds', 'AwayOdds']].values
        market_implied /= market_implied.sum(axis=1, keepdims=True)
        jsd = np.mean([jensenshannon(p, m)**2 for p, m in zip(probs, market_implied)])
        ...
        for i in range(10):
            ...
            if np.sum(in_bin) > 0:
                ...
                ece += np.abs(avg_confidence_in_bin - accuracy_in_bin) * (np.sum(in_bin) / len(probs))
...
def main():
    ...
    print("Task:   Walk-Forward Validation & Ablation Study")
    
    df = load_data()
    
    final_combined_results = None
    final_combined_le = None
    final_summary_combined = None
    
    for mode in ['odds_only', 'l5_only', 'combined']:
        print(f"\n\n================================================")
        print(f"=== RUNNING ABLATION MODE: {mode.upper()} ===")
        print(f"================================================")
        
        features, target = get_features_and_target(feature_mode=mode)
        ...
        if mode == 'combined':
            final_combined_results = results
            final_combined_le = le
            final_summary_combined = summary
            
    print("\nGenerating Scientific Visualizations (for combined mode)...")
    plot_calibration_curves(final_combined_results, final_combined_le)
    plot_temporal_roi(final_combined_results)
    plot_confusion_matrices(final_combined_results, final_combined_le)
    plot_bet_distribution(final_combined_results, final_combined_le)
    
    with open(OUTPUT_REPORT, "w") as f:
        f.write("WALK-FORWARD VALIDATION REPORT (COMBINED FEATURES)\n")
        f.write(final_summary_combined.to_string(index=False))
        f.write("\n\n INTERPRETATION OF METRICS:\n")
        f.write(" - Brier Score / LogLoss: Lower is better. Measures calibration accuracy.\n")
        f.write(" - ECE: Expected Calibration Error. Lower is better.\n")
        f.write(" - JSD: Divergence from market implied probabilities.\n")
